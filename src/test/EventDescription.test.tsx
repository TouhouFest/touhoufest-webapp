...
describe('EventDescription', () => {
    ...
    test("timezone-aware notification firing from event description header", async () => {
    ...
    test("back button doesnt trigger when default notification UI open", async () => {
        ...
        let callbackFxn:Function = () => {};
        let callBackConst:Function = () => {};
        // todo: change these to the app.addlistener variants
        App.addListener = vi.fn().mockImplementation(async (typeString:string, callBackNotify:Function) => {
            if(typeString === "backButton") {
                callbackFxn = callBackNotify;
            }
            return {remove: null}
        });
        App.removeAllListeners = vi.fn().mockImplementation(async () => {});

        let start_time:Dayjs = dayjs("3/5/2025 13:00");
        let event_package:any = {title: "foobar", uniqueID:0, combinedStart: start_time.toISOString()};
        let test_hide_fxn = vi.fn();
        let testEventDescriptionPage = <EventDescription show_var={true} hide_fxn={test_hide_fxn} event_package={event_package} evt_print={<></>} />
        render(testEventDescriptionPage);
